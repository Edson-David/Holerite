...
double horaExtra = Extra.horas(salario, horasTrabalhadas);
double horaExtra100 = Extra.horas100(salario, horasTrabalhadas);
double totalExtra = horaExtra + horaExtra100;
double dsr = totalExtra / 26 * 4;
bruto += dsr + totalExtra;

Console.WriteLine("Você possui bonificação?");
Console.WriteLine("Digite 1 para bônus sem ônus ou 2 para bônus com desconto");
input = Console.ReadLine();

if (!string.IsNullOrEmpty(input) && input == "1")
{
    double adicional1 = Adicionais.adicional();
    adicional = adicional1;
}
else if (!string.IsNullOrEmpty(input) && input == "2")
{
    double adicional1 = Adicionais.adicional();
    bruto += adicional1;
}
...
if (salario <= 1518)
{
    ...
    liquido = bruto - inss - descontoAdicional + adicional + totalExtra + dsr;
    Console.WriteLine($"O desconto do INSS ficou em R$ {inss.ToString("F2", CI)}");
    if (dsr > 0)
    {
        Console.WriteLine($"O total de horas extras foi de R$ {totalExtra.ToString("F2", CI)}");
        Console.WriteLine($"Seu DSR foi de R$ {dsr.ToString("F2", CI)}");
    }
...
else if (salario >= 1518.01 && salario <= 2793.88)
{
    ...
    liquido = bruto - inss - descontoAdicional + adicional;
    Console.WriteLine($"O desconto do INSS ficou em R$ {inss.ToString("F2", CI)}");
    if (dsr > 0)
    {
        Console.WriteLine($"O total de horas extras foi de R$ {totalExtra.ToString("F2", CI)}");
...
else if (salario >= 2793.89 && salario <= 4190.84)
{
    ...
    liquido = bruto - inss - descontoAdicional + adicional + horaExtra100 + horaExtra + dsr;
    Console.WriteLine($"O desconto do INSS ficou em R$ {inss.ToString("F2", CI)}");
    if (dsr > 0)
    {
        Console.WriteLine($"O total de horas extras foi de R$ {totalExtra.ToString("F2", CI)}");
